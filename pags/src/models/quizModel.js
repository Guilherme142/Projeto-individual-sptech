...
function cadastrar(acertos, erros,fkUsuario) {
    ...
    var instrucao = `
        INSERT INTO quiz (acertos, erros, dataQuiz,fkUsuario) VALUES ('${acertos}', '${erros}',now(),'${fkUsuario}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
...
function consultaDesempenho(fk) {
    console.log("ACESSEI O USUARIO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function entrar(): ")
    var instrucao = `
    SELECT dataQuiz,acertos,erros FROM quiz WHERE fkUsuario = ${fk};
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
